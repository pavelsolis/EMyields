...
forvalues region = 1/4 {
	if `region' == 1 local rgn "LA"
	if `region' == 2 local rgn "EE"
	if `region' == 3 local rgn "EA"
	if `region' == 4 local rgn "MA"
	...
	foreach t in 24 120 {
		...
		foreach v in `vars' {
			...
			forvalues h = 0/$horizon {
				...
				// conditions
				local condition em & regionem == `region'
...
// 				graph export $pathfigs/LPs/`shk'/`rgn'/`v'`t'm.eps, replace
				local graphs`shock'`rgn'`t' `graphs`shock'`rgn'`t'' `v'`t'm
				drop *_`shock'_`v'`t'm				// b_ and confidence intervals
			}	// `v' yield component

			graph combine `graphs`shock'`rgn'`t'', rows(1) ycommon
			graph export $pathfigs/LPs/`shk'/REGION/`shk'`rgn'nomyptpphi`t'm.eps, replace
			graph drop _all
			local graphs`shock'`rgn'`t'				// blanks out macro
		}		// `shock'
	}		// `t' tenor
}		// EM `region'
