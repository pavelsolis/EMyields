...
foreach gfctime in 0 1 {
	if `gfctime' == 0 {
		local condition em & date < td(1oct2008)	// & `region' == 4
		local suffix "Pre"
	}
	else {
		local condition em & date >= td(1oct2008)	// & `region' == 4
...
// 				graph export $pathfigs/LPs/`shk'/`grp'/`v'`t'm.eps, replace
			...
			graph export $pathfigs/LPs/`shk'/`grp'/`shk'`grp'nomyptpphi`t'm`suffix'.eps, replace
			graph drop _all
			local graphs`shock'`grp'`t'				// blanks out macro
		}		// `shock'
	}		// `t' tenor
}		// `gfctime' Pre or Post
